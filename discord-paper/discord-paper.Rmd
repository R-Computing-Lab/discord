...
```{r preview-pre-processed-data, echo = FALSE, eval = knitr::is_html_output(),error=TRUE}
flu_ses_data %>% 
...
```{r preview-linked-dat, echo = FALSE, eval = knitr::is_html_output(),error=TRUE}
...
```{r summarize-model-html, echo = FALSE, eval = knitr::is_html_output(),error=TRUE}
flu_model_output %>%
...
```{r summarize-model-latex, echo = FALSE, eval = knitr::is_latex_output(),error=TRUE}
flu_model_output %>%
  mutate(p.value = scales::pvalue(p.value, add_p = TRUE),
         across(.cols = where(is.numeric), ~round(.x, 3))) %>%
  rename("Standard Error" = std.error,
