...
This vignette provides a detailed guide for incorporating categorical predictors into discordant-kinship regression analyses using the `discord` package. We demonstrate how to structure, code, and interpret categorical variables that operate at either the dyad level (between-dyads) or vary across and within dyads (mixed variables).


To illustrate, we examine whether sex predicts SES at age 40, using different categorical coding schemes. The data come from a subset of the 1979 National Longitudinal Survey of Youth (NLSY79).

This vignette addresses that challenge by demonstrating how to apply discordant-kinship regression models using categorical predictors — specifically, sex and race. The approach draws on dyadic modeling strategies described in [@kenny2006] and incorporates their extension to the kinship setting following [@Hwang2022].


We now walk through the full analytic process, beginning with data preparation.

# Data Preparation

We load the necessary packages and data.
...
In dyadic analysis, categorical predictors can fall into three types:

- Between-dyad variables: constant within dyads (e.g., same race)

- Within-dyad variables: vary within dyads but constant across (rare for categories)
