...
To illustrate, we examine whether sex and race predict SES at age 40, using different categorical coding schemes with data from the 1979 National Longitudinal Survey of Youth (NLSY79). 
...
This distinction affects how we code and interpret these variables in regression models. 


## Variable Types and Their Implications

In dyadic analysis, variables are classified based on their variation patterns:
...
We then use `CreatePairLinksSingleEntered()` from the `NlsyLinks` package to merge kinship links with the target variables. This gives us a sibling-pair dataset in "wide" format, with suffixes distinguishing the two individuals in each pair.
...
To avoid violating independence assumptions, we retain only one sibling pair per household. This ensures each dyad is independent in the regression model.
...
### Brief explanation for types of variable in dyadic analysis
...
cat_both %>%
  ...
  summarize( n(),.groups = 'drop')%>%
  kableExtra::kbl('html', align = "c",
                  col.names=c("RACE_multi", "RACE_1","RACE_2","SEX_binary","SEX_multi","SEX_1","SEX_2",
  "sample_size"))%>%
  kableExtra::kable_styling(full_width = FALSE) 
...
This vignette has demonstrated how to incorporate categorical predictors in discordant-kinship regression analyses using the `discord` package. We've shown that:

1. Categorical variables can be handled as between-dyad or mixed variables depending on their distribution
2. The `discord_data()` function facilitates appropriate recoding of categorical predictors
3. Different coding schemes (binary and multi-category) offer flexibility based on research questions


The analyses revealed that race composition significantly predicted SES differences between siblings, while sex composition did not. These findings illustrate how the `discord` package allows researchers to examine complex research questions while maintaining appropriate controls for kinship structures.


For implementation in your own research, we recommend:
- Consider the theoretical nature of your categorical predictors
- Choose appropriate coding schemes based on your research questions
