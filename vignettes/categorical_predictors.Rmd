...
# We only pick one pair per household so we don't violate the independence assumption.
...
If the predictor is a continuous variable, the DV (the difference score of the outcome variable) is regressed on the mean score of the outcome variable, the mean score of the predictor, and the difference score of the predictor. However, it is impossible to calculate the mean score and difference score of the categorical variable. This vignette introduces how to deal with categorical predictors in discordant-kinship regressions. In this example, we use sex and race as categorical variables for demonstration. We referred to  @kenny2006  for idea about handling categorical predictors in dyadic analysis, and @Hwang2022 for incorporating that idea into discordant-kinship regression.

## Handling Categorical Predictors

## Mixed Variables: Gender as an Example

In kinship regression, a variable can be at the within-dyad level, the between-dyad level, or a combination. This combination is called a "mixed variable", meaning that it varies both within and between dyads. Gender is a prototypical example of a mixed variable.
...
The sex compositions in the data are as follows:
...
By doing so, the sex variable (which was initially a mixed variable) becomes a between-dyad variable as follows:
...
discord_cat_diff <- lm(S00_H40_diff ~  S00_H40_mean + RACE_multimatch,
    data = cat_race) 
```
...
discord_cat_mean <- lm( S00_H40_mean ~ RACE_multimatch + SEX_binarymatch,
    data = cat_both) 
...
discord_cat_diff <- lm(S00_H40_diff ~  S00_H40_mean + RACE_multimatch + SEX_multimatch,
    data = cat_both) 
