...
This vignette demonstrates how to incorporate categorical predictors in discordant-kinship regression analyses using the `discord` package.  Building on dyadic modeling strategies from Kenny et al. [@kenny2006] and extended to kinship models in our prior work [@Hwang2022], we present several approaches for handling categorical variables in these specialized regression models.
...
When analyzing continuous predictors, we typically calculate mean and difference scores. However, as Kenny et al. [@kenny2006] note, these calculations are not meaningful for categorical variables and must be replaced with appropriate coding strategies.
...
1. **Binary Match Coding**: Indicates whether both members of a pair share the same category (1) or differ (0).
