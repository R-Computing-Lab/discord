...
This vignette demonstrates how to incorporate categorical predictors in discordant-kinship regression analyses using the `discord` package.  Drawing on dyadic modeling strategies described by Kenny et al. [@kenny2006] and their extension to kinship models by Hwang and Garrison [@Hwang2022], we present several approaches for handling categorical variables in these specialized regression models.
...
We summarize these variable types in the table below:
...
When analyzing continuous predictors, we typically calculate mean scores and difference scores. However, as Kenny et al. [@kenny2006] note, categorical variables require alternative approaches, since mean and difference calculations are not meaningful in this context. Instead, we can use coding strategies to represent categorical variables in a way that allows for meaningful analysis.
...
Binary match coding generates a simple indicator of whether pairs match (1) or differ (0) on a categorical variable:
- Same-sex pairs (male-male or female-female) → 1
- Mixed-sex pairs (male-female) → 0

**When to use**: When your research question focuses on similarity versus difference, rather than on specific category effects.
...
**When to use**: When you hypothesize distinct effects for specific categories (e.g., male pairs vs. female pairs).

Following Hwang and Garrison [@Hwang2022], the approach you select should align with your research questions and theoretical framework.


# Data Preparation


We demonstrate categorical predictor handling using sibling data from the NLSY79. We begin by loading the necessary packages and preparing the dataset.
...
Next, we filter and clean the dataset, create kinship links, and recode the categorical variables. In this example, we use full siblings (R = 0.5) from the Gen1 cohort.
...
To ensure that the dependent variable (SES at age 40) is available for both members of the pair, we filter out cases with missing values. We also recode sex and race into factors. 
...
For full siblings, race a between-dyad variable. But we demonstrate below how one can restrict anaylses 
