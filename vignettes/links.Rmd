...
df_potter<- ped2fam(df_potter, famID = "famID", personID = "personID")
...
df_links <- com2links(writetodisk = FALSE, ad_ped_matrix = add,  cn_ped_matrix = cn,
                      drop_upper_triangular = TRUE) %>% filter(ID1!=ID2)

df_links %>% slice(1:10) %>% knitr::kable()
...
syn_df <- kinsim(mu_all = c(1,1), 
                 r_a=c(.125),r_c=c(0), 
                 cov_e = .4, nrow(df_cousin)) %>% select(-c(A1_1, A1_2, A2_1, A2_2, 
                                                         C1_1, C1_2, C2_1, C2_2,
                                                         E1_1, E1_2, E2_1, E2_2,
                                                         r
                                                         )) 
...
We then use discord_regression() to fit a discordant-kinship model, predicting y1 from y2. Based on the structure of the data, we expect that there will be a significant association between the two outcome variables, as there is a known coverlapping non-shared environment covariance 
