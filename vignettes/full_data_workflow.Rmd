...
## Visualizing the Ordering Process

To make the ordering process more concrete, let's examine a specific pair before and after transformation:

```{r visualize-ordering}
...
# Show transformed data for same pairs
df_discord_weight %>%
  slice(1:3) %>%
  select(
    id, weight_1, weight_2, weight_diff,
    height_1, height_2, height_diff
  ) %>%
  knitr::kable(
    digits = 2,
...
Notice how the ordering changes depending on which sibling has the higher outcome value:
...
This ordering ensures that `weight_diff` is always positive (or zero in case of ties), which is essential for the interpretation of the difference scores in the regression model.
...
## Understanding Sibling Selection vs. Outcome-Based Ordering

Before we proceed with the analyses, it's important to understand the distinction between two different ways of ordering siblings:

### Selection for Standard OLS (First-Born or Oldest Sibling)

When running a standard OLS regression, researchers typically select **one sibling per family** to avoid non-independence. Common approaches include:
...
This selection is fixed based on a **demographic characteristic** (birth order, age) and remains constant regardless of which outcome variable you're analyzing. This approach is used for individual-level regression to establish a baseline.

### Ordering for Discordant Models (Outcome-Based)

In contrast, `discord_data()` orders siblings based on the **outcome variable** itself:

- The sibling with the **higher outcome value** becomes `_1`
- The sibling with the **lower outcome value** becomes `_2`
- This ordering is **specific to each outcome variable** you analyze

**Why order by outcome?** This ensures that difference scores (`outcome_diff = outcome_1 - outcome_2`) are always positive (or zero), which:
1. Simplifies interpretation of regression coefficients
2. Makes the direction of effects clear (does the higher-outcome sibling also have higher predictors?)
3. Maintains statistical properties of the model

**Important:** The ordering is done separately for each outcome. If you analyze weight as an outcome, siblings are ordered by weight. If you analyze height as an outcome, they're ordered by height. The ordering will likely differ between analyses.

### Example: First-Born vs. Outcome-Ordered

Let's demonstrate both approaches:

```{r demonstrate-selection}
# Approach 1: Select first-born (or in this case, s1) for OLS
# This is fixed - always the same sibling regardless of outcome
df_fixed_selection <- df_synthetic %>%
  select(pid, weight_s1, height_s1, age_s1) %>%
  slice(1:3)

# Approach 2: Order by outcome (weight) for discord model  
# Changes based on who has higher weight
df_weight_ordered <- df_discord_weight %>%
  select(id, weight_1, weight_2, height_1, height_2) %>%
  slice(1:3)

# Approach 3: Order by different outcome (age) for different analysis
df_age_ordered <- df_discord_age %>%
  select(id, age_1, age_2, weight_1, height_1) %>%
  slice(1:3)

df_fixed_selection %>%
  knitr::kable(digits = 2, caption = "Fixed selection: Always use s1 for OLS")

df_weight_ordered %>%
  knitr::kable(digits = 2, caption = "Weight-ordered: Sibling with higher weight is always _1")

df_age_ordered %>%
  knitr::kable(digits = 2, caption = "Age-ordered: Sibling with higher age is always _1")
```

Notice that:
- In the fixed selection, we always use `_s1` (could represent first-born)
- In the weight-ordered data, `_1` is whoever weighs more  
- In the age-ordered data, `_1` is whoever is older
- The person who is `_1` in weight-ordered may NOT be `_1` in age-ordered

This distinction is crucial for understanding what each analysis tells us.
...
For our baseline analysis, we select one sibling from each pair. In the original dataset, s1 was defined as the older sibling, so we can simply choose the sibling labeled `_s1`. This represents a **fixed selection strategy** - we're always choosing the same individual from each pair regardless of outcome.
...
```
```{r ols-output, message=FALSE, warning=FALSE, results='asis'}
...
```
```{r between-output, results='asis', message=FALSE, warning=FALSE}
...
```
```{r discord-manual-output, results='asis', message=FALSE, warning=FALSE}
...
glance(discord_model) %>%
  ...
  knitr::kable(digits = 3)
```
```{r discord-compare, results='asis'}
...
```{r all-models-comparison, results='asis', message=FALSE, warning=FALSE}
stargazer::stargazer(
  ols_model,
  between_model,
  discord_model,
...
Let's break down what each model tells us:

### 1. Standard OLS Regression (Individual-Level)
...
- **Interpretation**: "Individuals with higher height/age tend to have higher weight"
- **Limitation**: Cannot distinguish whether the association is causal or due to shared family factors

### 2. Between-Family Regression (Family-Level Means)

- **Data**: Average values for each sibling pair
- **Level of analysis**: Family level  
- **What it captures**: How family-average predictors relate to family-average outcomes
- **Interpretation**: "Families with higher average height/age tend to have higher average weight"
- **Limitation**: Only captures between-family variation; tells us nothing about within-family effects

### 3. Discordant-Kinship Regression (Within-Family Differences)

- **Data**: Differences between siblings within families
...
- **Interpretation**: "Within sibling pairs sharing family background, the sibling with higher height/age tends to have higher weight"
- **Advantage**: Controls for unmeasured familial confounders, providing stronger evidence for (or against) causal effects

## Understanding the Difference Scores

The difference scores (`*_diff`) in the discordant model are the key to understanding within-family effects:

```{r explain-diff-scores}
# Show the relationship between individual values and difference scores
df_discord_weight %>%
  slice(1:3) %>%
  select(id, height_1, height_2, height_mean, height_diff) %>%
  mutate(
    calculation_mean = (height_1 + height_2) / 2,
    calculation_diff = height_1 - height_2,
    mean_matches = abs(height_mean - calculation_mean) < 0.01,
    diff_matches = abs(height_diff - calculation_diff) < 0.01
  ) %>%
  knitr::kable(
    digits = 2,
    caption = "Demonstrating calculation of mean and difference scores"
  )
```

**Interpretation guide:**

- **Positive `*_diff` coefficient**: The sibling with more of the predictor tends to have more of the outcome, even within the same family
- **Non-significant `*_diff` coefficient**: No within-family effect; the association seen in OLS is likely due to familial confounding
- **Negative `*_diff` coefficient**: The sibling with less of the predictor has more of the outcome (suggesting suppression effects)

## Practical Implications

The comparison of these three models helps researchers:

1. **Assess confounding**: Large differences between OLS and discordant estimates suggest familial confounding
2. **Test causality**: Significant within-family effects (difference scores) provide stronger evidence for causal relationships
3. **Understand mechanisms**: Comparing between-family and within-family effects reveals whether effects operate at the family or individual level

This multi-model approach is the essence of the discordant-kinship design, allowing researchers to separate within-family from between-family effects and better understand the nature of associations in their data.
