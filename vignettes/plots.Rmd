...
df_discord_flu <- df_discord_flu %>%
  mutate(
    ...
    ses_diff_group = factor(
      case_when(
        as.numeric(scale(s00_h40_diff)) > 0.33 ~ "More Advantage",
        as.numeric(scale(s00_h40_diff)) < -0.33 ~ "Less Advantage",
        abs(as.numeric(scale(s00_h40_diff))) <= 0.33 ~ "Equally Advantage"
      ),
      levels = c(
        "Less Advantaged",
        "Equally Advantaged",
