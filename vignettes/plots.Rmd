...
plot_btwn <- ggplot(df_discord_flu, aes(
  x = s00_h40_mean,
  y = flu_total_mean, fill = ses_diff_group#,
 # group = ses_diff_group
)) +
    geom_point(
    size = 1.8, alpha = 0.5, na.rm = TRUE,
   # aes(colour = s00_h40_diff),
    position = position_jitter(width = 0.2, height = 0.2)
  ) +
  geom_point(
    size = 0.8, alpha = 0.8, na.rm = TRUE,
...
plot_btwn
#ggMarginalGadget(plot_btwn)

# Set relative size of marginal plots (main plot 10x bigger than marginals)
ggMarginal(plot_btwn, type="histogram", size=10,groupColour = TRUE, groupFill = F)


ggMarginal(plot_btwn, type="density", size=10,groupColour = F, groupFill = T, aes( color = ses_diff_group))                                                          
#ggMarginal(plot_btwn, type="density", size=10,groupColour = T, groupFill = F)

# to do, figure out why the ses-diff-group coloring isn't working in the marginal plots right now they are all black
# this is also happening to the fake geom_point it should be colored by ses_diff_group but isn't
```
...
plot_within <- ggplot(df_discord_flu, aes(
  ...
  geom_point(
    size = 0.8, alpha = 0.8, na.rm = TRUE,
    position = position_jitter(weight = 0.2, height = 0.2)
  ) +
