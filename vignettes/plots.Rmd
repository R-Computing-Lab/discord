...
discord_flu <- discord_flu %>%
  mutate(
    ...
    ses_diff_group = factor(case_when(scale(s00_h40_diff) > 0.33 ~ "More Advantage",
...
individual_plot <- ggplot(flu_modeling_data, aes(x = s00_h40_s1,
                                                y = flu_total_s1,
...
# Main scatter plot
main_plot_within <- ggplot(discord_flu, aes(x = s00_h40_diff,
  ...
  scale_colour_gradientn(
    ...
    colours = shading,
    na.value = "#AD78B6",
    values = scales::rescale(c(min(discord_flu$s00_h40_diff, na.rm = TRUE),
